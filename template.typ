...
#let conf(author: "Fabio Lenherr", title, subtitle, doc) = {
  ...
  show raw.where(block: true): content => block(
    width: 100%, fill: luma(240), stroke: 1pt + maroon, radius: 3pt, inset: 5pt, clip: false, {
      let numbers = true
      let stepnumber = 1
      ...
      let (columns, align, make_row) = {
        if numbers {
          ...
          if type(numberstyle) == "auto" {
            numberstyle = text.with(style: "italic", slashed-zero: true, size: .6em)
          }
          (
      ...
      table(
        stroke: none, columns: columns, rows: (auto,), gutter: 0pt, inset: 2pt, align: (col, _) => align.at(col), ..content
        .text
        .split("\n")
        .enumerate()
        .map(make_row)
