...
Definition: *SPA is a website that fits on a single web page with the goal of providing a user experience similar to that of a desktop application.*\
In an SPA, either _all necessary code_ is retrieved with a _single page load_ or the appropriate resources are _dynamically laoded_ and added to the page as necessary.\
...
- Partitioning of client and server allows for *seperation of concerns* and *better maintainability*

#align(center, [#image("../Screenshots/2023_09_21_02_47_52.png", width: 70%)])
Bundling: In order to not send unnecessary data to the client, and to obfuscate code..., we bundle the entire page.
...
- routing is done solely on client
  - challange: how do we store URLs when we don't actually call anything?\
    ...
    - HTML5: window.history.pushState()\
      both of these forces the browser to remember the state of the SPA in order to restore last page
...
The idea is that instead of calling functions directly, we pass in code, that will later be called.\
Mostly this is done with other languages -> from JS we call html, from whatever we call SQL, etc\
However, this can also be used in SPA by using frameworks that offer integrated dependency injection.
#columns(2, [
